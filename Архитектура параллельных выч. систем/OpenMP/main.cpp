...
void count_state(int k) {
    ...
    /*int middle = (int)(cmax/2) + J_MAX/2;
    A.set(middle, middle, -1);
    A.set(middle, middle - 1, 1);
    c.set(middle, 0, 100 * D_Y);
*/
...
void main_part(int num_of_threads) {
    ...
    cout << "Time: " << end.tv_sec-start.tv_sec+0.000000001*(end.tv_nsec-start.tv_nsec) << "sec." << endl << endl;
}

int main (int argc, char** argv) {
    vector<int> var_num_of_threads = {1, 2, 4, 8};

    for (int num_of_threads : var_num_of_threads) {
        main_part(num_of_threads);
    }
